...
void glfw_init(const glfw_desc_t* desc) {
  ...
  glfwWindowHint(
      GLFW_SAMPLES, (desc_def.sample_count == 1) ? 0 : desc_def.sample_count
  );
  ...
  _window =
      glfwCreateWindow(desc_def.width, desc_def.height, desc_def.title, 0, 0);
  glfwMakeContextCurrent(_window);
  glfwSwapInterval(1);
}

GLFWwindow* glfw_window(void) { return _window; }
      ...
      .defaults =
          {
              ...
              .depth_format = _no_depth_buffer ? SG_PIXELFORMAT_NONE
